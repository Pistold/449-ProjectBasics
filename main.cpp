...
Scene lightHouse() {
	// This scene is more complicated; it has child objects, as well as animators.
	auto lightHouse = assimpLoad("models/lightHouse/scene.gltf", true);
	lightHouse.move(glm::vec3(0, 0, 0));
	lightHouse.grow(glm::vec3(0.05, 0.05, 0.05));
	...
	// move that vector as part of the return value.
	std::vector<Object3D> objects;
	objects.push_back(std::move(lightHouse));

	// We want these animations to referenced the *moved* objects, which are no longer
	// in the variables named "tiger" and "boat". "boat" is now in the "objects" list at
	// index 0, and "tiger" is the index-1 child of the boat.

	Animator animLighHouse;
	//animLighHouse.addAnimation(std::make_unique<RotationAnimation>(objects[0], 10, glm::vec3(0, 6.28, 0)));
	
	// The Animators will be destroyed when leaving this function, so we move them into
	// a list to be returned.
	std::vector<Animator> animators;
	animators.push_back(std::move(animLighHouse));
...
Scene lightHouse() {
	...
	//boat
	auto Boat = assimpLoad("models/boat/lightHouse.gltf", true);
	Boat.move(glm::vec3(0, 0, 0));
	Boat.grow(glm::vec3(0.05, 0.05, 0.05));

	//birds
	auto Birds = assimpLoad("models/Birds/birds.gltf", true);
	Birds.move(glm::vec3(0, 0, 0));
	Birds.grow(glm::vec3(0.05, 0.05, 0.05));
	
	//lighthouse light
	auto Light = assimpLoad("models/Light/llight.gltf", true);
	Light.move(glm::vec3(0, 0, 0));
	Light.grow(glm::vec3(0.05, 0.05, 0.05));

	//Crab and Lighthouse
	auto Crab = assimpLoad("models/Crab/crab.gltf", true);
	Crab.move(glm::vec3(0, 0, 0));
	Crab.grow(glm::vec3(0.05, 0.05, 0.05));

	//water
	auto Water = assimpLoad("models/Water/water.gltf", true);
	Water.move(glm::vec3(0, 0, 0));
	Water.grow(glm::vec3(0.05, 0.05, 0.05));


	//adding objects
	std::vector<Object3D> objects;
	objects.push_back(std::move(Boat));
	objects.push_back(std::move(Birds));
...
int main() {
	...
	*/
	...
	auto scene = lightHouse();
	auto& lightHouse = scene.objects[0];

	auto cameraPosition = glm::vec3(0, 3, 3);
	auto camera = glm::lookAt(cameraPosition, glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));
	auto perspective = glm::perspective(glm::radians(45.0), static_cast<double>(window.getSize().x) / window.getSize().y, 0.1, 100.0);
